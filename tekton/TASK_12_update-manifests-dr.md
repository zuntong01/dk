## __`TASK_12_update-manifests-dr.md`__

#### <b><span style="color:cyan">[TASK 설명]</span></b>  
```bash
gitops repo에서 가져온 manifest 파일을 context_dir(dr) 디렉토리의 deployment.yaml 파일에 이미지명을 신규빌드한 컨테이너이미지로 업데이트(수정)
```
...
# deployment.yaml 의 이미지명 업데이트를 위한, gitops 디렉토리(예: prd, test, dev, dr 등), 신규이미지명을 파라메터로 정의
  ...
  steps:
    ...
    ## 신규빌드한 이미지명으로 -->> gitops repo에서 clone한 prd(환경별다름)/deployment.yaml 의 이미지명을 치환함
      ...
      ## gitops repo에서 가져온 dr디렉토리의 deployment.yaml 파일에 이미지명을 파이프라인에서 만든 이미지로 변경함
      set -x
