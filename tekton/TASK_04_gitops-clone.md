## __`TASK_04_gitops-clone.md`__

#### <b><span style="color:cyan">[TASK 설명]</span></b>  
```bash
Gitlab repo(project:MSE_GOS)에서 --> Tekton TASK 내부의 Workspace 로 git-init 명령으로 배포관린 yaml (예: deploytment.yaml, route.yaml, hpa.yaml, secret 등) 파일을 가져옴 (신규빌드한 정보로 업데이트 하기위한 사전준비)
```
